...
def summarize_results(scored: pd.DataFrame, chargebacks: pd.DataFrame) -> pd.DataFrame:
    ...
    known_fraud = scored.merge(
        chargebacks[["transaction_id"]], on="transaction_id", how="left", indicator=True
    )
    known_fraud["is_chargeback"] = (known_fraud["_merge"] == "both").astype(int)

    fraud_by_label = known_fraud.groupby("risk_label", as_index=False).agg(
